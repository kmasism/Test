...
namespace UpgradeHelpers.DB
{
    ...
    public class AdoNetHelper
    {
        ...
        {
            if (dataTable == null) throw new ArgumentNullException("dataTable is null");
        ...
        {
            if (dataSet == null) throw new ArgumentNullException("dataSet is null");
        ...
        /// <returns>A byte[] containing the serialized data.</returns>
        public static byte[] SerializeDataSet(DataSet dataSet, bool optimizeForSize = true)
        {
            if (dataSet == null) throw new ArgumentNullException("dataSet is null");

            return new FastSerializer().Serialize(dataSet, optimizeForSize);
        }
        ...
        {
            if (dataSetType == null)
                throw new ArgumentNullException("dataSetType is null");
            if (serializedData == null)
                throw new ArgumentNullException("serializedData is null");
        ...
        {
            if (dataSet == null) throw new ArgumentNullException("dataSet is null");
            if (serializedData == null) throw new ArgumentNullException("serializedData is null");
            if (dataSet.GetType() == typeof(DataSet)) throw new ArgumentException("Is not a typed DataSet", "dataSet");
        ...
        {
            if (dataTableType == null) throw new ArgumentNullException("dataTableType is null");
            if (serializedData == null) throw new ArgumentNullException("serializedData is null");
        ...
        {
            if (dataSetType == null) throw new ArgumentNullException("dataSetType is null");
            if (serializedData == null) throw new ArgumentNullException("serializedData is null");
        ...
        public static object[] GetKeyValuesFromRow(DataRow row, DataTable primaryKeyProvider)
        {
            ...
            if (primaryKeyProvider == null)
                throw new ArgumentNullException("primaryKeyProvider is null");
        ...
        {
            if (rows == null) throw new ArgumentNullException("rows is null");
            if (columnName == null) throw new ArgumentNullException("columnName is null");
        ...
        {
            if (rows == null) throw new ArgumentNullException("rows is null");
            if (column == null) throw new ArgumentNullException("column is null");
        ...
        public static BitVector32 GetDataSetFlags(DataSet dataSet)
        {
        ...
        private static BitVector32 GetTableFlags(DataTable dataTable)
        {
			...
			if (TableTypeNameFieldInfo != null)
			{
				flags[TableHasTypeName] = (TableTypeNameFieldInfo.GetValue(dataTable) as XmlQualifiedName) != null;
			}
			...
			if (TableRepeatableElementFieldInfo != null)
			{
        ...
        private class FastSerializer
        {
            ...
            #region Methods
            public byte[] Serialize(DataSet dataSet, bool optimizeForSize = true)
            {
                this.dataSet = dataSet;
                writer = new SerializationWriter();

                writer.OptimizeForSize = optimizeForSize;
            ...
            private void serializeColumns(DataTable dataTable)
            {
                ...
                foreach (DataColumn column in columns)
                {
                    ...
                    {
						//mobilize-note: dperez, why save the absolute value?
						//writer.WriteOptimized(Math.Abs(column.AutoIncrementSeed));
						//writer.WriteOptimized(Math.Abs(column.AutoIncrementStep));
						writer.WriteOptimized(column.AutoIncrementSeed);
						writer.WriteOptimized(column.AutoIncrementStep);
					}
        ...
        private class FastDeserializer
        {
            ...
            public DataTable DeserializeSimpleTypedDataTable(DataTable dataTable, byte[] serializedData)
            {
                ...
                for (int i = 0; i < rowCount; i++)
                {
					...
					{
						addColumnsToDataTable(dataTable, values.Length - dataTable.Columns.Count);
					}
			...
			private void addColumnsToDataTable(DataTable dataTable, int columnCount)
			{
            ...
            private void deserializeRows(DataTable dataTable)
            {
                ...
                for (int i = 0; i < rowCount; i++)
                {
					...
					if (!flags[RowHasOldData])
					{
						...
						row = dataTable.LoadDataRow(values, !flags[RowHasNewData]);
					}
					else if (!flags[RowHasNewData])
					{
            ...
            private void deserializeColumns(DataTable dataTable)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        column.AutoIncrementSeed = reader.ReadOptimizedInt64();
						column.AutoIncrementStep = reader.ReadOptimizedInt64();
                    }
