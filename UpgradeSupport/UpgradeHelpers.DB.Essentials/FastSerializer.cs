    ...
    public sealed class SerializationWriter : BinaryWriter
    {
        ...
        public void WriteObject(object value)
        {
            ...
            else if (value is bool)
            {
                ...
                var typed = boolValue ? SerializedType.BooleanTrueType : SerializedType.BooleanFalseType;
                writeTypeCode(typed);
            }
            ...
            else
            {
				...
				// Get and store information about the object's type
				string valueTypeStr = value.GetType().AssemblyQualifiedName;
				byte[] nameBytes = Encoding.UTF8.GetBytes(valueTypeStr);
				Write(nameBytes.Length);
				Write(nameBytes);
				// Store the object as an array of bytes
...
#region Private Methods
        ...
        private object processObject(SerializedType typeCode)
        {
			...
			{
				// Load object type name
				int typeNameLength = ReadInt32();
				byte[] typeNameBytes = ReadBytes(typeNameLength);
				string typeName = Encoding.UTF8.GetString(typeNameBytes);
				int xmlLength = ReadInt32();
				// Load the object
				using (MemoryStream ms = new MemoryStream())
				{
					ms.Write(ReadBytes(xmlLength), 0, xmlLength);
					ms.Seek(0, SeekOrigin.Begin);
					System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(Type.GetType(typeName));
