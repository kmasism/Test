...
namespace UpgradeHelpers.DB
{
    ...
    public class FieldsHelper : IEnumerable<FieldHelper>
    {
        ...
        /// Add a column
        /// </summary>
        public void Append(string name, System.Data.DbType type, int definedSize = 0, FieldAttributeEnum attributes = FieldAttributeEnum.adFldUnspecified, params object[] restParams)
        {
            DataColumn newColumn = new DataColumn(name, DBUtils.GetType(type))
            {
                AllowDBNull = (attributes != FieldAttributeEnum.adFldUnspecified) && Convert.ToBoolean(attributes & FieldAttributeEnum.adFldIsNullable)
            };
            rs.FieldsMetadata.Add(newColumn);
        }
