...
namespace UpgradeHelpers.Gui.Controls
{
    ...
    public partial class CommandButtonHelper : Component, IExtenderProvider, ISupportInitialize
    {
        ...
        public static void SetDisabledPictureProperty(Button button, Image image)
        {
            ...
            if (image != null)
                button.EnabledChanged += Button_EnabledChanged;
            if (!button.Enabled)//Needed to avoid an exception when the enabled property was setted to false before the EnabledChanged event overload
            {
        ...
        public static void SetStyleProperty(bool designMode, Button button, int style)
        {
            ...
            if (Styles[button] == 1)
            {
                if(DisabledPictures.ContainsKey(button) && !button.Enabled)//Set the image when it is a disabledPicture case.
                    button.Image = DisabledPictures[button];
                else if (UpPictures.ContainsKey(button))
