...
namespace UpgradeHelpers.Gui.Controls
{
    ...
    public partial class ImageListHelper : UserControl
    {
        ...
        /// <summary>
        /// Returns the image from a key (string) or index (integer)
        /// </summary>
        /// <param name="imageList">The list of images</param>
        /// <param name="keyOrIndex">The key or index used to retrieve an image</param>
        /// <returns>Image from key or index</returns>
        public static Image GetImageItem(ImageList imageList, object keyOrIndex)
        {
            Image image = null;
            if(keyOrIndex != null)
            {
                if (keyOrIndex.GetType().Name.ToLower().Equals("string"))
                    image = imageList.Images[keyOrIndex.ToString()];
                else
                    image = imageList.Images[Convert.ToInt32(keyOrIndex) - 1];
            }
            return image;
        }
