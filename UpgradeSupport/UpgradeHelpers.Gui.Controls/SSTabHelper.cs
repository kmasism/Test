...
namespace UpgradeHelpers.Gui.Controls
{
    ...
    public partial class SSTabHelper : Component, ISupportInitialize, IExtenderProvider
    {
        ...
        {
            var customTabControl = tabControl as TabControlExtension;
            if (customTabControl != null)
            {
                int key = customTabControl.GetHashCode();
                if (value)
                {
                    if (!ControlOfCustomDrawingMode.ContainsKey(key))
                    {
                        customTabControl.DrawMode = TabDrawMode.OwnerDrawFixed;
                        customTabControl.CustomDrawItem += TabControl_DrawItem;
                ...
                else
                {
                    if (ControlOfCustomDrawingMode.ContainsKey(key))
                    {
                        ...
                        {
                            customTabControl.DrawMode = TabDrawMode.Normal;
                            customTabControl.DrawItem -= TabControl_DrawItem;
