...
namespace UpgradeHelpers.Gui.Controls
{
    ...
    public static class TreeViewHelper
    {
        ...
        /// <summary>
        /// Returns a Node object based on its visible position within the control.
        /// </summary>
        /// <param name="instance">The TreeView itself</param>
        /// <param name="Index">Index of the node to be found</param>
        /// <returns>The node if it exists, Null otherwise</returns>
        public static TreeNode GetVisibleNode(this TreeView instance, short Index)
        {
            List<TreeNode> visibleNodes = null;
            try
            {
                var node = instance.Nodes.Cast<TreeNode>().Where(x => x.IsVisible).FirstOrDefault();
                visibleNodes = new List<TreeNode>();
                while (node != null)
                {
                    var temp = node;
                    node = node.NextVisibleNode;
                    visibleNodes.Add(temp);
                }
                return visibleNodes.Count() != 0 && Index <= visibleNodes.Count() ? visibleNodes[Index] : null;
            }
            catch (Exception)
            {
                return null;
